...
description: 当用户输入 /cm、cm、commit msg，或要求提交暂存变更时，准备并创建 Conventional Commits。检查暂存区，按 package 或功能边界分组，保留未暂存工作；完成提交后默认推送远端，最多尝试 5 次。
---
...
8. 创建全部提交后，默认执行 `git push`，除非用户明确要求不要推送。
9. 推送失败时可以重试，但总尝试次数最多为 5 次（首次执行计入 5 次）。
